...
Original file is located at
    https://colab.research.google.com/drive/1HNE9JRErt5DEUmqo6LmyTXRIBJmSP0Wr
"""



"""# Task
Build a Series A startup financial model in Python using Pandas and Plotly. The model should include the following modules: Revenue Forecast (with inputs for starting revenue, monthly growth, churn, ARPU, and outputs for 24-month forecast, scenario toggles, chart), Unit Economics Stress Test (with inputs for CAC, LTV, fixed/variable cost per unit, and outputs for LTV/CAC ratio, contribution margin, stress grid), Cap Table Simulator (with inputs for pre-money valuation, round size, current cap table (founders, pre-seed, ESOP), post-round ESOP %, and outputs for post-money cap table, share price, dilution breakdown chart), and Term Sheet Comparison (with inputs for 2–3 offer scenarios (valuation + round size), and outputs for investor %, founder %, post-money, side-by-side bar chart). Deliver a Colab-ready notebook with clean sections, editable inputs, and visual outputs.
